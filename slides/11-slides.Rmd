...
---
## Recipe 3.1: Code for Redo Recipe 3
...
ratings_bonanza8 <- ratings %>% 
  ...
  spread(episode, viewers_7day) %>% 
  mutate(finale_bump = last - first,
         finale_bump_median = finale_bump - median(finale_bump),
...
---
## Recipe 8: Code for Bar Chart, Take 2!
...
ratings_bonanza9 <- ratings %>% 
  ...
  group_by(series) %>% 
  filter(episode == 1 | episode == max(episode)) %>% 
  mutate(pct_change = (viewers_7day - lag(viewers_7day)) /
           lag(viewers_7day)) %>% 
  ungroup()
...
ggplot(ratings_bonanza9, aes(x = fct_rev(series), 
  ...
  geom_col(fill = beyonce_palette(49)[1], alpha = .5) +
  coord_flip() +
  labs(x = "Series", y = "Percentage Change in Viewers from First to Last Episode") +
  theme_minimal() +
  ggtitle("Percent Increase in Viewers was the Smallest for Series 8",
          subtitle= "Finale 7-day Viewers Relative to Premiere, Line is the Median") +
  theme(text = element_text(family = "Lato")) +
...
---
## Recipe 9: Code for % Change Bar Chart
...
ggplot(ratings_bonanza9, aes(x = fct_rev(series), 
                             y = pct_change)) +
  geom_point(color = beyonce_palette(49)[1]) +
...
---
## Recipe 10: Code for Lollipop Plot, % Change
