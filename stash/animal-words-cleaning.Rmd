...
```{r}
# my animals vector
sticker_animals <- c("cat", "kitty", "chicken (animal)", 
                "hen", "dog", "puppy")

# first get the items in the animal category
animals <- get_item_data(language = "English (American)", 
                         form = "WS") %>%
  filter(definition %in% sticker_animals) 
...
# how many kids produce either of each word?
animal_produced <- animal_data %>% 
...
animal_summary <- animal_produced %>% 
...
```{r}
write_csv(animal_data, here::here("data", "animal_data.csv"))
write_csv(animal_summary, here::here("data", "animal_summary.csv"))
```
