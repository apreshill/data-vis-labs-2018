...
At this point, our plot is looking pretty good. But you may have noticed that the legend order doesn't match the lines. We can fix that using the [`forcats` package](http://forcats.tidyverse.org), which is `for` `cat`egorical variables. There are lots of functions in `forcats`. We'll use the `fct_reorder2` function, which works for when you have a line chart of two quantitative variables, colored by a factor variable. Let's see the difference.
